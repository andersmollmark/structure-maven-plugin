...
import structure.analyze.functionalInterface.ImportFilter;
import structure.analyze.functionalInterface.PrefixType;
...
public class StructureAnalyzer {
    ...
    public List<String> getAnalyzedResultAF1(String filename, String path) throws IOException {
        ...
        List<String> warnings =
                theReader.readFile().
                        filter(oneRow -> {
                            boolean importrad = oneRow.startsWith(IMPORT);
                            if(importrad){
                                System.out.println("En rad:" + oneRow);
                            }
                            return importrad;
                        }).
                        map(aImportRow -> {
